...
export default function CoreValue() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from(".corevalue-text", {
        immediateRender: false, opacity: 0, y: 22, duration: 0.7, ease: "power3.out",
        scrollTrigger: { trigger: ".corevalue-text", start: "top 85%", toggleActions: "play none none none" },
      });
      gsap.from(".corevalue-visual", {
        immediateRender: false, opacity: 0, scale: 0.96, duration: 0.8, ease: "power3.out",
        scrollTrigger: { trigger: ".corevalue-visual", start: "top 85%", toggleActions: "play none none none" },
      });
    }, sectionRef);
  ...
  return (
    <section ref={sectionRef} className="py-20 md:py-28 bg-white">
      <div className="container-x grid lg:grid-cols-12 gap-10 items-center">
        <div className="corevalue-text lg:col-span-6">
          ...
          <h2 className="font-display mt-3 text-3xl md:text-4xl lg:text-5xl text-[var(--color-primary)] leading-tight">
            ...
            <span className="italic text-[var(--color-secondary-dark)]">Financial Advisory</span>
          </h2>
          ...
          <ul className="mt-7 grid sm:grid-cols-2 gap-3">
            {points.map((p) => (
              <li key={p} className="flex items-start gap-2.5 text-sm text-[var(--color-ink)]/85">
        ...
        <div className="corevalue-visual lg:col-span-6">
          <CoreVisual />
        </div>
      </div>
    </section>
...
function CoreVisual() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      const bars = visualRef.current?.querySelectorAll(".cv-bar");
      const nums = visualRef.current?.querySelectorAll<HTMLElement>(".cv-num");
      const targets = [98, 86, 94];
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          bars?.forEach((bar, i) => {
            gsap.fromTo(bar, { width: "0%" }, { width: `${targets[i]}%`, duration: 1.4, delay: i * 0.15, ease: "power3.out" });
          });
          nums?.forEach((el, i) => {
            const obj = { val: 0 };
            gsap.to(obj, {
              val: targets[i], duration: 1.4, delay: i * 0.15, ease: "power3.out",
        ...
        <div className="rounded-[calc(1.5rem-2px)] bg-white p-6 md:p-8">
          ...
          <div className="mt-5 grid grid-cols-3 gap-3">
            {[
              ...
              <div key={m.k} className="rounded-xl bg-[var(--color-tertiary)] p-3">
                ...
                <p className="cv-num font-display text-2xl text-[var(--color-primary)] mt-0.5">0%</p>
                <div className="mt-2 h-1.5 rounded-full bg-white overflow-hidden">
                  <div className="cv-bar h-full gradient-gold" style={{ width: "0%" }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 rounded-xl border border-[var(--color-line)] p-4">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs font-semibold text-[var(--color-success)]">↑ 12.4%</p>
            </div>
            <svg viewBox="0 0 240 60" className="mt-2 w-full h-14">
              <path
                d="M0 50 L40 38 L80 42 L120 28 L160 30 L200 18 L240 22"
                fill="none" stroke="#D4A24C" strokeWidth="2.5" strokeLinecap="round"
              />
              <path
                d="M0 55 L40 48 L80 50 L120 40 L160 44 L200 36 L240 38"
                fill="none" stroke="#0A2540" strokeWidth="2" strokeOpacity="0.4"
